...
const ModalReportLessor = ({ closeModal }: { closeModal: () => void }) => {
  ...
  const reasons = stateReport.reasonsLessor.list.map((item) => {
    return { label: item.title, value: item.title };
  });

  React.useEffect(() => {
    (async () => {
      await actionReport.getListReasonAsync({ type: ENUM_TYPE_REASON.LESSOR });
    })();
      ...
      <form className="my-[15px]" onSubmit={handleSubmit(handleConfirm)}>
        <div className="">
          <SelectBoxField
            ...
            name="reasonLessor"
            registerForm={register('reasonLessor', {
        ...
        <div className="mt-[20px]">
          ...
          <LimitedTextArea
            ...
            limit={600}
            value=""
            placeholder="Mô tả lý do báo cáo người môi giới"
          />
...
export default ModalReportLessor;
