...
export const DISTRICTS = LOCATIONS.map((e) => {
  return { label: e.name, value: e.code };
});

export const WARDS = (district: number | undefined) => {
  if (district) {
    const districtFound = LOCATIONS.find((e) => e.code === district);
    if (districtFound) {
      return districtFound.wards.map((e) => {
        return { label: e.name, value: e.code };
      });
    }
  }
